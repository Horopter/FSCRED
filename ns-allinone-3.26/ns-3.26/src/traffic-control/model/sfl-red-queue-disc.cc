...
NS_LOG_COMPONENT_DEFINE ("SflRedQueueDisc");

NS_OBJECT_ENSURE_REGISTERED (SflRedQueueDisc);

SflRedQueueDisc::Stats m_sflstats;
...
std::vector<SFLResult> MemberShipOfAQL(double aql)
{
...
std::vector<SFLResult> MembershipOfMaxP(double maxp)
{
	double few_low1 = 0;
...
double MembershipOfDropP(std::vector<SFLResult> fr)
{
	double zero_low1 = 0;
	...
	double high_peak2 = 1;
	double high_low2 = 1;
	SFLResult zero,low,moderate,high;
	for(std::vector<SFLResult>::iterator it = fr.begin() ; it != fr.end(); ++it)
	{
		if(it->name == "zero")
		{
			zero.name = "zero";
...
SFLResult FIRB(SFLResult aql, SFLResult maxP)
{
	SFLResult fr;
...
std::vector<SFLResult> AvrgClassifier(std::vector<SFLResult> fr)
{
...
double InputDefuzzifier(std::vector<SFLResult> aql, std::vector<SFLResult> maxP)
{
	std::vector<SFLResult> result;
	for (std::vector<SFLResult>::iterator it1 = aql.begin() ; it1 != aql.end(); ++it1)
		for (std::vector<SFLResult>::iterator it2 = maxP.begin() ; it2 != maxP.end(); ++it2)
		{
			SFLResult f = FIRB(*it1, *it2);
			if(f.value>0)
				result.push_back(f);
		}
	result = AvrgClassifier(result);
	return MembershipOfDropP(result);	
}


TypeId SflRedQueueDisc::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::SflRedQueueDisc")
    .SetParent<QueueDisc> ()
    .SetGroupName("TrafficControl")
    .AddConstructor<SflRedQueueDisc> ()
    .AddAttribute ("Mode",
                   "Determines unit for QueueLimit",
                   EnumValue (Queue::QUEUE_MODE_PACKETS),
                   MakeEnumAccessor (&SflRedQueueDisc::SetMode),
                   MakeEnumChecker (Queue::QUEUE_MODE_BYTES, "QUEUE_MODE_BYTES",
                                    Queue::QUEUE_MODE_PACKETS, "QUEUE_MODE_PACKETS"))
    .AddAttribute ("MeanPktSize",
                   "Average of packet size",
                   UintegerValue (500),
                   MakeUintegerAccessor (&SflRedQueueDisc::m_meanPktSize),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("IdlePktSize",
                   "Average packet size used during idle times. Used when m_cautions = 3",
                   UintegerValue (0),
                   MakeUintegerAccessor (&SflRedQueueDisc::m_idlePktSize),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("Wait",
                   "True for waiting between dropped packets",
                   BooleanValue (true),
                   MakeBooleanAccessor (&SflRedQueueDisc::m_isWait),
                   MakeBooleanChecker ())
    .AddAttribute ("Gentle",
                   "True to increases dropping probability slowly when average queue exceeds maxthresh",
                   BooleanValue (true),
                   MakeBooleanAccessor (&SflRedQueueDisc::m_isGentle),
                   MakeBooleanChecker ())
    .AddAttribute ("ARED",
                   "True to enable ARED",
                   BooleanValue (false),
                   MakeBooleanAccessor (&SflRedQueueDisc::m_isARED),
                   MakeBooleanChecker ())
    .AddAttribute ("AdaptMaxP",
                   "True to adapt m_curMaxP",
                   BooleanValue (false),
                   MakeBooleanAccessor (&SflRedQueueDisc::m_isAdaptMaxP),
                   MakeBooleanChecker ())
    .AddAttribute ("SFLRED",
                   "True to enable SFLRED",
                   BooleanValue (false),
                   MakeBooleanAccessor (&SflRedQueueDisc::m_isSFLRED),
                   MakeBooleanChecker ()) 
    .AddAttribute ("MinTh",
                   "Minimum average length threshold in packets/bytes",
                   DoubleValue (5),
                   MakeDoubleAccessor (&SflRedQueueDisc::m_minTh),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("MaxTh",
                   "Maximum average length threshold in packets/bytes",
                   DoubleValue (15),
                   MakeDoubleAccessor (&SflRedQueueDisc::m_maxTh),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("QueueLimit",
                   "Queue limit in bytes/packets",
                   UintegerValue (25),
                   MakeUintegerAccessor (&SflRedQueueDisc::SetQueueLimit),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("QW",
                   "Queue weight related to the exponential weighted moving average (EWMA)",
                   DoubleValue (0.002),
                   MakeDoubleAccessor (&SflRedQueueDisc::m_qW),
                   MakeDoubleChecker <double> ())
    .AddAttribute ("LInterm",
                   "The maximum probability of dropping a packet",
                   DoubleValue (50),
                   MakeDoubleAccessor (&SflRedQueueDisc::m_lInterm),
                   MakeDoubleChecker <double> ())
    .AddAttribute ("TargetDelay",
                   "Target average queuing delay in ARED",
                   TimeValue (Seconds (0.005)),
                   MakeTimeAccessor (&SflRedQueueDisc::m_targetDelay),
                   MakeTimeChecker ())
    .AddAttribute ("Interval",
                   "Time interval to update m_curMaxP",
                   TimeValue (Seconds (0.5)),
                   MakeTimeAccessor (&SflRedQueueDisc::m_interval),
                   MakeTimeChecker ())
    .AddAttribute ("Top",
                   "Upper bound for m_curMaxP in ARED",
                   DoubleValue (0.5),
                   MakeDoubleAccessor (&SflRedQueueDisc::m_top),
                   MakeDoubleChecker <double> (0, 1))
    .AddAttribute ("Bottom",
                   "Lower bound for m_curMaxP in ARED",
                   DoubleValue (0.0),
                   MakeDoubleAccessor (&SflRedQueueDisc::m_bottom),
                   MakeDoubleChecker <double> (0, 1))
    .AddAttribute ("Alpha",
                   "Increment parameter for m_curMaxP in ARED",
                   DoubleValue (0.01),
                   MakeDoubleAccessor (&SflRedQueueDisc::SetAredAlpha),
                   MakeDoubleChecker <double> (0, 1))
    .AddAttribute ("Beta",
                   "Decrement parameter for m_curMaxP in ARED",
                   DoubleValue (0.9),
                   MakeDoubleAccessor (&SflRedQueueDisc::SetAredBeta),
                   MakeDoubleChecker <double> (0, 1))
    .AddAttribute ("SFLBeta",
                   "Increment parameter for m_curMaxP in SFL's Adaptive RED",
                   DoubleValue (2.0),
                   MakeDoubleAccessor (&SflRedQueueDisc::SetSFLREDB),
                   MakeDoubleChecker <double> ())
    .AddAttribute ("SFLAlpha",
                   "Decrement parameter for m_curMaxP in SFL's Adaptive RED",
                   DoubleValue (3.0),
                   MakeDoubleAccessor (&SflRedQueueDisc::SetSFLREDA),
                   MakeDoubleChecker <double> ())
    .AddAttribute ("LastSet",
                   "Store the last time m_curMaxP was updated",
                   TimeValue (Seconds (0.0)),
                   MakeTimeAccessor (&SflRedQueueDisc::m_lastSet),
                   MakeTimeChecker ())
    .AddAttribute ("Rtt",
                   "Round Trip Time to be considered while automatically setting m_bottom",
                   TimeValue (Seconds (0.1)),
                   MakeTimeAccessor (&SflRedQueueDisc::m_rtt),
                   MakeTimeChecker ())
    .AddAttribute ("Ns1Compat",
                   "NS-1 compatibility",
                   BooleanValue (false),
                   MakeBooleanAccessor (&SflRedQueueDisc::m_isNs1Compat),
                   MakeBooleanChecker ())
    .AddAttribute ("LinkBandwidth", 
                   "The RED link bandwidth",
                   DataRateValue (DataRate ("1.5Mbps")),
                   MakeDataRateAccessor (&SflRedQueueDisc::m_linkBandwidth),
                   MakeDataRateChecker ())
    .AddAttribute ("LinkDelay", 
                   "The RED link delay",
                   TimeValue (MilliSeconds (20)),
                   MakeTimeAccessor (&SflRedQueueDisc::m_linkDelay),
                   MakeTimeChecker ())
  ;
...
SflRedQueueDisc::SflRedQueueDisc () :
  QueueDisc ()
{
...
void
SflRedQueueDisc::DoDispose (void)
{
  NS_LOG_FUNCTION (this);
  m_uv = 0;
  QueueDisc::DoDispose ();
...
void
SflRedQueueDisc::SetMode (Queue::QueueMode mode)
{
...
void
SflRedQueueDisc::SetAredAlpha (double alpha)
{
...
double
SflRedQueueDisc::GetAredAlpha (void)
{
  NS_LOG_FUNCTION (this);
  return m_alpha;
}
...
void
SflRedQueueDisc::SetAredBeta (double beta)
{
...
double
SflRedQueueDisc::GetAredBeta (void)
{
  NS_LOG_FUNCTION (this);
  return m_beta;
}
...
double
SflRedQueueDisc::GetSFLREDA (void)
{
  NS_LOG_FUNCTION (this);
  return m_a;
}
...
void
SflRedQueueDisc::SetSFLREDA (double a)
{
...
double
SflRedQueueDisc::GetSFLREDB (void)
{
  NS_LOG_FUNCTION (this);
  return m_b;
}
...
void
SflRedQueueDisc::SetSFLREDB (double b)
{
...
void
SflRedQueueDisc::SetQueueLimit (uint32_t lim)
{
...
void
SflRedQueueDisc::SetTh (double minTh, double maxTh)
{
...
SflRedQueueDisc::Stats
SflRedQueueDisc::GetStats ()
{
  NS_LOG_FUNCTION (this);
  return m_sflstats;
}
...
int64_t 
SflRedQueueDisc::AssignStreams (int64_t stream)
{
...
bool
SflRedQueueDisc::DoEnqueue (Ptr<QueueDiscItem> item)
{
  ...
  aql.minTh = m_minTh;
  aql.maxTh = m_maxTh;
  m_sflstats.avgQLen.push_back(aql);

  m_qAvg = Estimator (nQueued, m + 1, m_qAvg, m_qW);

  aql.qlen = m_qAvg;
  m_sflstats.curQLen.push_back(aql);
 ...
 */
void
SflRedQueueDisc::InitializeParams (void)
{
  NS_LOG_FUNCTION (this);
  ...
  m_sflstats.forcedDrop = 0;
  m_sflstats.unforcedDrop = 0;
  m_sflstats.qLimDrop = 0;
...
void
SflRedQueueDisc::UpdateMaxPSFL (double newAve)
{
...
void
SflRedQueueDisc::UpdateMaxP (double newAve, Time now)
{
  double m_part = 0.4 * (m_maxTh - m_minTh);
...
double
SflRedQueueDisc::Estimator (uint32_t nQueued, uint32_t m, double qAvg, double qW)
{
  ...
  if (m_isAdaptMaxP && now > m_lastSet + m_interval)
    ...
    {
      UpdateMaxPSFL (newAve);  // Update MaxP using SCRED in MIMD fashion.
    }
...
uint32_t
SflRedQueueDisc::DropEarly (Ptr<QueueDiscItem> item, uint32_t qSize)
{
...
double
SflRedQueueDisc::CalculatePNew (double qAvg, double maxTh, bool isGentle, double vA,
...
*/
	...
	std::vector<SFLResult> qAvgFuzzy = MemberShipOfAQL(qAvg);
	std::vector<SFLResult> maxPFuzzy = MembershipOfMaxP(maxP);
...
// Returns a probability using these function parameters for the DropEarly funtion
double 
SflRedQueueDisc::ModifyP (double p, uint32_t count, uint32_t countBytes,
...
uint32_t
SflRedQueueDisc::GetQueueSize (void)
{
  NS_LOG_FUNCTION (this);
  if (GetMode () == Queue::QUEUE_MODE_BYTES)
    {
...
Ptr<QueueDiscItem>
SflRedQueueDisc::DoDequeue (void)
{
  NS_LOG_FUNCTION (this);
...
SflRedQueueDisc::DoPeek (void) const
{
  NS_LOG_FUNCTION (this);
  if (GetInternalQueue (0)->IsEmpty ())
    {
...
bool
SflRedQueueDisc::CheckConfig (void)
{
  NS_LOG_FUNCTION (this);
  if (GetNQueueDiscClasses () > 0)
    {
      NS_LOG_ERROR ("SflRedQueueDisc cannot have classes");
      return false;
    }

  if (GetNPacketFilters () > 0)
    {
      NS_LOG_ERROR ("SflRedQueueDisc cannot have packet filters");
      return false;
    }
  ...
  if (GetNInternalQueues () != 1)
    {
      NS_LOG_ERROR ("SflRedQueueDisc needs 1 internal queue");
      return false;
    }

  if (GetInternalQueue (0)->GetMode () != m_mode)
    {
      NS_LOG_ERROR ("The mode of the provided queue does not match the mode set on the SflRedQueueDisc");
      return false;
    }
