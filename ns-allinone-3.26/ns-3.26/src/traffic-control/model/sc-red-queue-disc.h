#ifndef SCRED_QUEUE_DISC_H
#define SCRED_QUEUE_DISC_H
 ...
 *
 * \brief A SCRED packet queue disc
 */
...
class qTimeSCRED
  {
	...
	uint32_t minTh;
	uint32_t maxTh;
	friend std::ostream& operator<<(std::ostream&, const qTimeSCRED&);
  };

std::ostream& operator<<(std::ostream& stream, const qTimeSCRED& obj)
{   
...
class SCRedQueueDisc : public QueueDisc
{
public:
  /**
  ...
  /**
   * \brief SCRedQueueDisc Constructor
   *
   * Create a SCRED queue disc
   */
  SCRedQueueDisc ();
   ...
   * Destructor
   */ 
  virtual ~SCRedQueueDisc ();

  /** 
   * \brief Used in SC's Adaptive SCRED
   */
  typedef enum 
    {
  ...
  {   
    ...
    std::vector<qTimeSCRED> avgQLen;
    std::vector<qTimeSCRED> curQLen;
  } Stats;
  ...
  uint32_t GetQueueSize (void);
   ...
   /**
    * \brief Set the alpha value to adapt m_curMaxP in SC's Adaptive SCRED.
    *
    * \param alpha The value of alpha to adapt m_curMaxP in SC's Adaptive SCRED.
    */
   void SetSCREDA (double a);

   /**
    * \brief Get the alpha value to adapt m_curMaxP in SC's Adaptive SCRED.
    *
    * \returns The alpha value to adapt m_curMaxP in SC's Adaptive SCRED.
    */
   double GetSCREDA (void);

   /**
    * \brief Set the beta value to adapt m_curMaxP in SC's Adaptive SCRED.
    *
    * \param beta The value of beta to adapt m_curMaxP in SC's Adaptive SCRED.
    */
   void SetSCREDB (double b);

   /**
    * \brief Get the beta value to adapt m_curMaxP in SC's Adaptive SCRED.
    *
    * \returns The beta value to adapt m_curMaxP in SC's Adaptive SCRED.
    */
   double GetSCREDB (void);
  ...
  /**
   * \brief Set the thresh limits of SCRED.
   *
  ...
  /**
   * \brief Get the SCRED statistics after running.
   *
...
private:
   ...
   * simulation, the bandwidth-dependent SCRED parameters do not change.
  ...
  void UpdateMaxP (double newAve, Time now);
   /**
    * \brief Update m_curMaxP in SC's Adaptive SCRED
    * \param newAve new average queue length
    */
  void UpdateMaxPSC (double newAve);
  /**
  ...
  Stats m_scstats; //!< SCRED statistics
  ...
  bool m_isARED;            //!< True to enable Adaptive SCRED
  ...
  bool m_isSCRED;    //!< True to enable SC's Adaptive SCRED
  double m_b;               //!< Increment parameter for m_curMaxP in SC's Adaptive SCRED
  double m_a;               //!< Decrement parameter for m_curMaxP in SC's Adaptive SCRED
  bool m_isNs1Compat;       //!< Ns-1 compatibility
  DataRate m_linkBandwidth; //!< Link bandwidth
  Time m_linkDelay;         //!< Link delay

  // ** Variables maintained by SCRED
  ...
  Status_t m_status;        //!< For use in SC's Adaptive SCRED  
  /**
   * 0 for default SCRED
