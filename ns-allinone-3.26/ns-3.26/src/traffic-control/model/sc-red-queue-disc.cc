...
#include "ns3/abort.h"
#include "sc-red-queue-disc.h"
#include "ns3/drop-tail-queue.h"

namespace ns3 {

NS_LOG_COMPONENT_DEFINE ("SCRedQueueDisc");

NS_OBJECT_ENSURE_REGISTERED (SCRedQueueDisc);

SCRedQueueDisc::Stats m_scstats;
...
std::vector<SCResult> MemberShipOfAQL(double aql)
{
...
std::vector<SCResult> MembershipOfMaxP(double maxp)
{
	double few_low1 = 0;
...
double MembershipOfDropP(std::vector<SCResult> fr)
{
	double zero_low1 = 0;
	...
	double high_peak2 = 1;
	double high_low2 = 1;
	SCResult zero,low,moderate,high;
	for(std::vector<SCResult>::iterator it = fr.begin() ; it != fr.end(); ++it)
	{
		if(it->name == "zero")
		{
			zero.name = "zero";
...
SCResult FIRB(SCResult aql, SCResult maxP)
{
	SCResult fr;
...
std::vector<SCResult> AvrgClassifier(std::vector<SCResult> fr)
{
...
double InputDefuzzifier(std::vector<SCResult> aql, std::vector<SCResult> maxP)
{
	std::vector<SCResult> result;
	for (std::vector<SCResult>::iterator it1 = aql.begin() ; it1 != aql.end(); ++it1)
		for (std::vector<SCResult>::iterator it2 = maxP.begin() ; it2 != maxP.end(); ++it2)
		{
			SCResult f = FIRB(*it1, *it2);
			if(f.value>0)
				result.push_back(f);
		}
...
TypeId SCRedQueueDisc::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::SCRedQueueDisc")
    .SetParent<QueueDisc> ()
    .SetGroupName("TrafficControl")
    .AddConstructor<SCRedQueueDisc> ()
    .AddAttribute ("Mode",
                   "Determines unit for QueueLimit",
                   EnumValue (Queue::QUEUE_MODE_PACKETS),
                   MakeEnumAccessor (&SCRedQueueDisc::SetMode),
                   MakeEnumChecker (Queue::QUEUE_MODE_BYTES, "QUEUE_MODE_BYTES",
                                    Queue::QUEUE_MODE_PACKETS, "QUEUE_MODE_PACKETS"))
    .AddAttribute ("MeanPktSize",
                   "Average of packet size",
                   UintegerValue (500),
                   MakeUintegerAccessor (&SCRedQueueDisc::m_meanPktSize),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("IdlePktSize",
                   "Average packet size used during idle times. Used when m_cautions = 3",
                   UintegerValue (0),
                   MakeUintegerAccessor (&SCRedQueueDisc::m_idlePktSize),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("Wait",
                   "True for waiting between dropped packets",
                   BooleanValue (true),
                   MakeBooleanAccessor (&SCRedQueueDisc::m_isWait),
                   MakeBooleanChecker ())
    .AddAttribute ("Gentle",
                   "True to increases dropping probability slowly when average queue exceeds maxthresh",
                   BooleanValue (true),
                   MakeBooleanAccessor (&SCRedQueueDisc::m_isGentle),
                   MakeBooleanChecker ())
    .AddAttribute ("ARED",
                   "True to enable ARED",
                   BooleanValue (false),
                   MakeBooleanAccessor (&SCRedQueueDisc::m_isARED),
                   MakeBooleanChecker ())
    .AddAttribute ("AdaptMaxP",
                   "True to adapt m_curMaxP",
                   BooleanValue (false),
                   MakeBooleanAccessor (&SCRedQueueDisc::m_isAdaptMaxP),
                   MakeBooleanChecker ())
    .AddAttribute ("SCRED",
                   "True to enable SCRED",
                   BooleanValue (false),
                   MakeBooleanAccessor (&SCRedQueueDisc::m_isSCRED),
                   MakeBooleanChecker ()) 
    .AddAttribute ("MinTh",
                   "Minimum average length threshold in packets/bytes",
                   DoubleValue (5),
                   MakeDoubleAccessor (&SCRedQueueDisc::m_minTh),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("MaxTh",
                   "Maximum average length threshold in packets/bytes",
                   DoubleValue (15),
                   MakeDoubleAccessor (&SCRedQueueDisc::m_maxTh),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("QueueLimit",
                   "Queue limit in bytes/packets",
                   UintegerValue (25),
                   MakeUintegerAccessor (&SCRedQueueDisc::SetQueueLimit),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("QW",
                   "Queue weight related to the exponential weighted moving average (EWMA)",
                   DoubleValue (0.002),
                   MakeDoubleAccessor (&SCRedQueueDisc::m_qW),
                   MakeDoubleChecker <double> ())
    .AddAttribute ("LInterm",
                   "The maximum probability of dropping a packet",
                   DoubleValue (50),
                   MakeDoubleAccessor (&SCRedQueueDisc::m_lInterm),
                   MakeDoubleChecker <double> ())
    .AddAttribute ("TargetDelay",
                   "Target average queuing delay in ARED",
                   TimeValue (Seconds (0.005)),
                   MakeTimeAccessor (&SCRedQueueDisc::m_targetDelay),
                   MakeTimeChecker ())
    .AddAttribute ("Interval",
                   "Time interval to update m_curMaxP",
                   TimeValue (Seconds (0.5)),
                   MakeTimeAccessor (&SCRedQueueDisc::m_interval),
                   MakeTimeChecker ())
    .AddAttribute ("Top",
                   "Upper bound for m_curMaxP in ARED",
                   DoubleValue (0.5),
                   MakeDoubleAccessor (&SCRedQueueDisc::m_top),
                   MakeDoubleChecker <double> (0, 1))
    .AddAttribute ("Bottom",
                   "Lower bound for m_curMaxP in ARED",
                   DoubleValue (0.0),
                   MakeDoubleAccessor (&SCRedQueueDisc::m_bottom),
                   MakeDoubleChecker <double> (0, 1))
    .AddAttribute ("Alpha",
                   "Increment parameter for m_curMaxP in ARED",
                   DoubleValue (0.01),
                   MakeDoubleAccessor (&SCRedQueueDisc::SetAredAlpha),
                   MakeDoubleChecker <double> (0, 1))
    .AddAttribute ("Beta",
                   "Decrement parameter for m_curMaxP in ARED",
                   DoubleValue (0.9),
                   MakeDoubleAccessor (&SCRedQueueDisc::SetAredBeta),
                   MakeDoubleChecker <double> (0, 1))
    .AddAttribute ("SCBeta",
                   "Increment parameter for m_curMaxP in SC's Adaptive RED",
                   DoubleValue (2.0),
                   MakeDoubleAccessor (&SCRedQueueDisc::SetSCREDB),
                   MakeDoubleChecker <double> ())
    .AddAttribute ("SCAlpha",
                   "Decrement parameter for m_curMaxP in SC's Adaptive RED",
                   DoubleValue (3.0),
                   MakeDoubleAccessor (&SCRedQueueDisc::SetSCREDA),
                   MakeDoubleChecker <double> ())
    .AddAttribute ("LastSet",
                   "Store the last time m_curMaxP was updated",
                   TimeValue (Seconds (0.0)),
                   MakeTimeAccessor (&SCRedQueueDisc::m_lastSet),
                   MakeTimeChecker ())
    .AddAttribute ("Rtt",
                   "Round Trip Time to be considered while automatically setting m_bottom",
                   TimeValue (Seconds (0.1)),
                   MakeTimeAccessor (&SCRedQueueDisc::m_rtt),
                   MakeTimeChecker ())
    .AddAttribute ("Ns1Compat",
                   "NS-1 compatibility",
                   BooleanValue (false),
                   MakeBooleanAccessor (&SCRedQueueDisc::m_isNs1Compat),
                   MakeBooleanChecker ())
    .AddAttribute ("LinkBandwidth", 
                   "The RED link bandwidth",
                   DataRateValue (DataRate ("1.5Mbps")),
                   MakeDataRateAccessor (&SCRedQueueDisc::m_linkBandwidth),
                   MakeDataRateChecker ())
    .AddAttribute ("LinkDelay", 
                   "The RED link delay",
                   TimeValue (MilliSeconds (20)),
                   MakeTimeAccessor (&SCRedQueueDisc::m_linkDelay),
                   MakeTimeChecker ())
  ;
...
void
SCRedQueueDisc::SetMode (Queue::QueueMode mode)
{
...
void
SCRedQueueDisc::SetAredAlpha (double alpha)
{
...
double
SCRedQueueDisc::GetAredAlpha (void)
{
  NS_LOG_FUNCTION (this);
  return m_alpha;
}
...
void
SCRedQueueDisc::SetAredBeta (double beta)
{
...
double
SCRedQueueDisc::GetAredBeta (void)
{
  NS_LOG_FUNCTION (this);
  return m_beta;
}
...
void
SCRedQueueDisc::SetSCREDA (double a)
{
...
void
SCRedQueueDisc::SetSCREDB (double b)
{
...
void
SCRedQueueDisc::SetQueueLimit (uint32_t lim)
{
...
void
SCRedQueueDisc::SetTh (double minTh, double maxTh)
{
...
SCRedQueueDisc::Stats
SCRedQueueDisc::GetStats ()
{
  NS_LOG_FUNCTION (this);
  return m_scstats;
}
...
int64_t 
SCRedQueueDisc::AssignStreams (int64_t stream)
{
...
bool
SCRedQueueDisc::DoEnqueue (Ptr<QueueDiscItem> item)
{
  ...
  aql.minTh = m_minTh;
  aql.maxTh = m_maxTh;
  m_scstats.avgQLen.push_back(aql);

  m_qAvg = Estimator (nQueued, m + 1, m_qAvg, m_qW);

  aql.qlen = m_qAvg;
  m_scstats.curQLen.push_back(aql);
 ...
 */
void
SCRedQueueDisc::InitializeParams (void)
{
  NS_LOG_FUNCTION (this);
  ...
  std::cout << "InitParams :: RedMinth : " << m_minTh << " RedMaxTh : " << m_maxTh << " m_isSCRED :  "<< m_isSCRED <<std::endl;
  ...
  m_scstats.forcedDrop = 0;
  m_scstats.unforcedDrop = 0;
  m_scstats.qLimDrop = 0;
...
void
SCRedQueueDisc::UpdateMaxPSC (double newAve)
{
...
void
SCRedQueueDisc::UpdateMaxP (double newAve, Time now)
{
  double m_part = 0.4 * (m_maxTh - m_minTh);
...
double
SCRedQueueDisc::Estimator (uint32_t nQueued, uint32_t m, double qAvg, double qW)
{
  ...
  if (m_isAdaptMaxP && now > m_lastSet + m_interval)
    ...
    {
      UpdateMaxPSC (newAve);  // Update MaxP using SCRED in MIMD fashion.
    }
...
uint32_t
SCRedQueueDisc::DropEarly (Ptr<QueueDiscItem> item, uint32_t qSize)
{
...
double
SCRedQueueDisc::CalculatePNew (double qAvg, double maxTh, bool isGentle, double vA,
...
*/
	...
	std::vector<SCResult> qAvgFuzzy = MemberShipOfAQL(qAvg);
	std::vector<SCResult> maxPFuzzy = MembershipOfMaxP(maxP);
...
// Returns a probability using these function parameters for the DropEarly funtion
double 
SCRedQueueDisc::ModifyP (double p, uint32_t count, uint32_t countBytes,
...
uint32_t
SCRedQueueDisc::GetQueueSize (void)
{
  NS_LOG_FUNCTION (this);
  if (GetMode () == Queue::QUEUE_MODE_BYTES)
    {
...
SCRedQueueDisc::DoPeek (void) const
{
  NS_LOG_FUNCTION (this);
  if (GetInternalQueue (0)->IsEmpty ())
    {
...
bool
SCRedQueueDisc::CheckConfig (void)
{
  NS_LOG_FUNCTION (this);
  if (GetNQueueDiscClasses () > 0)
    {
      NS_LOG_ERROR ("SCRedQueueDisc cannot have classes");
      return false;
    }

  if (GetNPacketFilters () > 0)
    {
      NS_LOG_ERROR ("SCRedQueueDisc cannot have packet filters");
      return false;
    }
  ...
  if (GetNInternalQueues () != 1)
    {
      NS_LOG_ERROR ("SCRedQueueDisc needs 1 internal queue");
      return false;
    }

  if (GetInternalQueue (0)->GetMode () != m_mode)
    {
      NS_LOG_ERROR ("The mode of the provided queue does not match the mode set on the SCRedQueueDisc");
      return false;
    }
