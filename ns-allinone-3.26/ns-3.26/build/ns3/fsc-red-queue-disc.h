#ifndef FSCRED_QUEUE_DISC_H
#define FSCRED_QUEUE_DISC_H
 ...
 *
 * \brief A FSCRED packet queue disc
 */
...
class qTimeFSCRED
  {
	...
	uint32_t minTh;
	uint32_t maxTh;
	friend std::ostream& operator<<(std::ostream&, const qTimeFSCRED&);
  };

std::ostream& operator<<(std::ostream& stream, const qTimeFSCRED& obj)
{   
...
class FscRedQueueDisc : public QueueDisc
{
public:
  /**
  ...
  /**
   * \brief FscRedQueueDisc Constructor
   *
   * Create a FSCRED queue disc
   */
  FscRedQueueDisc ();
   ...
   * Destructor
   */ 
  virtual ~FscRedQueueDisc ();

  /** 
   * \brief Used in FSC's Adaptive FSCRED
   */
  typedef enum 
    {
  ...
  {   
    ...
    std::vector<qTimeFSCRED> avgQLen;
    std::vector<qTimeFSCRED> curQLen;
  } Stats;
  ...
  uint32_t GetQueueSize (void);
   ...
   /**
    * \brief Set the alpha value to adapt m_curMaxP in FSC's Adaptive FSCRED.
    *
    * \param alpha The value of alpha to adapt m_curMaxP in FSC's Adaptive FSCRED.
    */
   void SetFSCA (double a);

   /**
    * \brief Get the alpha value to adapt m_curMaxP in FSC's Adaptive FSCRED.
    *
    * \returns The alpha value to adapt m_curMaxP in FSC's Adaptive FSCRED.
    */
   double GetFSCA (void);

   /**
    * \brief Set the beta value to adapt m_curMaxP in FSC's Adaptive FSCRED.
    *
    * \param beta The value of beta to adapt m_curMaxP in FSC's Adaptive FSCRED.
    */
   void SetFSCB (double b);

   /**
    * \brief Get the beta value to adapt m_curMaxP in FSC's Adaptive FSCRED.
    *
    * \returns The beta value to adapt m_curMaxP in FSC's Adaptive FSCRED.
    */
   double GetFSCB (void);
  ...
  /**
   * \brief Set the thresh limits of FSCRED.
   *
  ...
  /**
   * \brief Get the FSCRED statistics after running.
   *
...
private:
   ...
   * simulation, the bandwidth-dependent FSCRED parameters do not change.
  ...
  void UpdateMaxP (double newAve, Time now);
   /**
    * \brief Update m_curMaxP in FSCRED
    * \param newAve new average queue length
    */
  void UpdateMaxPFSC (double newAve, Time now);
  /**
  ...
  Stats m_fscstats; //!< FSCRED statistics
  ...
  bool m_isFSC;    //!< True to enable FSCRED
  double m_b;               //!< Increment parameter for m_curMaxP in FSC's Adaptive FSCRED
  double m_a;               //!< Decrement parameter for m_curMaxP in FSC's Adaptive FSCRED
  bool m_isNs1Compat;       //!< Ns-1 compatibility
  DataRate m_linkBandwidth; //!< Link bandwidth
  Time m_linkDelay;         //!< Link delay

  // ** Variables maintained by FSCRED
  ...
  Status_t m_status;        //!< For use in FSC's Adaptive FSCRED  
  /**
   * 0 for default FSCRED
