...
int main (int argc, char *argv[])
{
  ...
  if ((queueDiscType != "RED") && (queueDiscType != "SFLRED"))
    {
      std::cout << "Invalid queue disc type: Use --queueDiscType=RED or --queueDiscType=SFLRED" << std::endl;
      exit (1);
    }
  ...
  if (!modeBytes)
    {
      Config::SetDefault ("ns3::SflRedQueueDisc::Mode", StringValue ("QUEUE_MODE_PACKETS"));
      Config::SetDefault ("ns3::SflRedQueueDisc::QueueLimit", UintegerValue (queueDiscLimitPackets));
    }
  else
    {
      Config::SetDefault ("ns3::SflRedQueueDisc::Mode", StringValue ("QUEUE_MODE_BYTES"));
      Config::SetDefault ("ns3::SflRedQueueDisc::QueueLimit", UintegerValue (queueDiscLimitPackets * pktSize));
      minTh *= pktSize;
      maxTh *= pktSize;
    }

  Config::SetDefault ("ns3::SflRedQueueDisc::MinTh", DoubleValue (minTh));
  Config::SetDefault ("ns3::SflRedQueueDisc::MaxTh", DoubleValue (maxTh));
  Config::SetDefault ("ns3::SflRedQueueDisc::LinkBandwidth", StringValue (bottleNeckLinkBw));
  Config::SetDefault ("ns3::SflRedQueueDisc::LinkDelay", StringValue (bottleNeckLinkDelay));
  Config::SetDefault ("ns3::SflRedQueueDisc::MeanPktSize", UintegerValue (pktSize));

  if (queueDiscType == "SFLRED")
    {
      // Turn on SFLRED
  ...
  SflRedQueueDisc::Stats st = StaticCast<SflRedQueueDisc> (queueDiscs.Get (0))->GetStats ();
  ...
  std::ostream_iterator<qTimeSFLRED> output_iterator1(output_file1, "\n");
  ...
  std::ostream_iterator<qTimeSFLRED> output_iterator2(output_file2, "\n");
