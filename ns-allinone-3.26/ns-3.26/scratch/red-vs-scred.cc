...
int main (int argc, char *argv[])
{
  ...
  if ((queueDiscType != "RED") && (queueDiscType != "SCRED"))
    {
      std::cout << "Invalid queue disc type: Use --queueDiscType=RED or --queueDiscType=SCRED" << std::endl;
      exit (1);
    }
  ...
  if (!modeBytes)
    {
      Config::SetDefault ("ns3::SCRedQueueDisc::Mode", StringValue ("QUEUE_MODE_PACKETS"));
      Config::SetDefault ("ns3::SCRedQueueDisc::QueueLimit", UintegerValue (queueDiscLimitPackets));
    }
  else
    {
      Config::SetDefault ("ns3::SCRedQueueDisc::Mode", StringValue ("QUEUE_MODE_BYTES"));
      Config::SetDefault ("ns3::SCRedQueueDisc::QueueLimit", UintegerValue (queueDiscLimitPackets * pktSize));
      minTh *= pktSize;
      maxTh *= pktSize;
    }

  Config::SetDefault ("ns3::SCRedQueueDisc::MinTh", DoubleValue (minTh));
  Config::SetDefault ("ns3::SCRedQueueDisc::MaxTh", DoubleValue (maxTh));
  Config::SetDefault ("ns3::SCRedQueueDisc::LinkBandwidth", StringValue (bottleNeckLinkBw));
  Config::SetDefault ("ns3::SCRedQueueDisc::LinkDelay", StringValue (bottleNeckLinkDelay));
  Config::SetDefault ("ns3::SCRedQueueDisc::MeanPktSize", UintegerValue (pktSize));

  if (queueDiscType == "SCRED")
    {
      // Turn on SCRED
  ...
  SCRedQueueDisc::Stats st = StaticCast<SCRedQueueDisc> (queueDiscs.Get (0))->GetStats ();
  ...
  std::ostream_iterator<qTimeSCRED> output_iterator1(output_file1, "\n");
  ...
  std::ostream_iterator<qTimeSCRED> output_iterator2(output_file2, "\n");
