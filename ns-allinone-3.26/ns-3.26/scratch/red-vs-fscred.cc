...
int main (int argc, char *argv[])
{
  ...
  cmd.AddValue ("queueDiscType", "Set Queue disc type to FSCRED or RED", queueDiscType);
  ...
  if ((queueDiscType != "RED") && (queueDiscType != "FSCRED"))
    {
      std::cout << "Invalid queue disc type: Use --queueDiscType=RED or --queueDiscType=FSCRED" << std::endl;
      exit (1);
    }
  ...
  if (!modeBytes)
    {
      Config::SetDefault ("ns3::FscRedQueueDisc::Mode", StringValue ("QUEUE_MODE_PACKETS"));
      Config::SetDefault ("ns3::FscRedQueueDisc::QueueLimit", UintegerValue (queueDiscLimitPackets));
    }
  else
    {
      Config::SetDefault ("ns3::FscRedQueueDisc::Mode", StringValue ("QUEUE_MODE_BYTES"));
      Config::SetDefault ("ns3::FscRedQueueDisc::QueueLimit", UintegerValue (queueDiscLimitPackets * pktSize));
      minTh *= pktSize;
      maxTh *= pktSize;
    }

  Config::SetDefault ("ns3::FscRedQueueDisc::MinTh", DoubleValue (minTh));
  Config::SetDefault ("ns3::FscRedQueueDisc::MaxTh", DoubleValue (maxTh));
  Config::SetDefault ("ns3::FscRedQueueDisc::LinkBandwidth", StringValue (bottleNeckLinkBw));
  Config::SetDefault ("ns3::FscRedQueueDisc::LinkDelay", StringValue (bottleNeckLinkDelay));
  Config::SetDefault ("ns3::FscRedQueueDisc::MeanPktSize", UintegerValue (pktSize));

  if (queueDiscType == "FSCRED")
    {
      // Turn on FSCRED
  ...
  FscRedQueueDisc::Stats st = StaticCast<FscRedQueueDisc> (queueDiscs.Get (0))->GetStats ();
  ...
  std::ostream_iterator<qTimeFSCRED> output_iterator1(output_file1, "\n");
  ...
  std::ostream_iterator<qTimeFSCRED> output_iterator2(output_file2, "\n");
